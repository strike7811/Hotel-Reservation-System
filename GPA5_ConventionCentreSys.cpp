...
int validateRDate(int RDate, bool moreCheck)
{
    ...
    do
    {
        ...
        {                          // do the following division operations to extract the individual digits of the date
            ...
            if (YY >= (systemYear - 2000)) // if year is at least starting from system year
            {
            ...
            {
                cout << "             Error! Year of Reservation Date must not be before the year of entry date!\n"; // display error message
...
void confirmReservation()
{
    ...
    while (confirmOUTERLOOP == 0)
    {
        ...
        else
        {
            ...
            {
                cout << "                                Returning back to Process Reservation Menu...\n"; // Display message return back to process menu
                cout << "                                ";
                confirmOUTERLOOP = 1;
            }
            else
                                ...
                                if (payment_option == 'Y')
                                {
                                    ...
                                    while (validateoption == true) // while validateoption equal to true
                                    {
                                        ...
                                        if (option == 'N')        // if option equal to 'N'
                                        {
                                            ...
                                            cout << "                                Please inform the customer to confirm the reservation"
                                                 << "\n                              before the actual day of reservation.\n\n";
                                            cout << "\n                                Returning back to Process Reservation Menu..." << endl;
                                            cout << "                                ";
...
void revenuebycustomer()
{
    ...
    CustomerRevenue custArray[100]; // Declare Customer Array to store all customer names
        ...
        if (currentCust.reservStatus != 'X') // If the status is not 'X' which means cancelled
            {
                ...
                custArray[count] = revenueCust; // store revenueCust into custArray with specific position
                ++count;
            }
    ...
    */
    ...
    //*****************************
    char tempCust[1];
    int tempCustRevAmt, tempCustStatus;
    ...
    int g = 0;           // initialize variable g to 0
    while (g < count) // while g less than recCount (which is total number of customer excluding cancelled status)
    {
        ...
        if (strcmp(custArray[g + 1].RevCustSort, custArray[g + 2].RevCustSort) == 0) // Compare Customer B and Customer C if they are the same customer
        {
            ...
            count -= 1; // Redule recCount by one value as same Customer data is merged
        }
        else
        {
